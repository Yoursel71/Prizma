...
public sealed class ProfileCatalog
{
    ...
    public IReadOnlyList<ZapretProfile> Load(string directory)
    {
    ...
    private static ZapretProfile LoadProfile(string path)
    {
        using var stream = File.OpenRead(path);
        var profile = JsonSerializer.Deserialize<ZapretProfile>(stream, SerializerOptions)
