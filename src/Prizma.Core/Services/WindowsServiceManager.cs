...
public sealed class WindowsServiceManager
{
    ...
    public const string LegacyServiceName = "ZapretTR.Engine";
    ...
    public WindowsServiceManager(string? programDataDirectory = null)
    {
        ...
        _installDirectory = Path.Combine(Path.GetFullPath(root), "Prizma", "engine");
    }
    ...
    public async Task<WindowsServiceState> GetStateAsync(CancellationToken cancellationToken = default)
        => await GetStateAsync(ServiceName, cancellationToken);

    private static async Task<WindowsServiceState> GetStateAsync(string serviceName, CancellationToken cancellationToken)
    {
    ...
    public async Task InstallAsync(string sourceEngineDirectory, ConnectionProfile profile, CancellationToken cancellationToken = default)
    {
        ...
        if (await GetStateAsync(cancellationToken) != WindowsServiceState.NotInstalled)
            throw new InvalidOperationException("Prizma hizmeti zaten kurulu.");
        if (await GetStateAsync(LegacyServiceName, cancellationToken) != WindowsServiceState.NotInstalled)
            throw new InvalidOperationException("Eski ZapretTR hizmeti kurulu. Önce eski sürümden hizmeti kaldırın.");
        ...
        await RunScAsync(["create", ServiceName, "binPath=", binaryPath, "start=", "auto", "DisplayName=", "Prizma Bağlantı Koruması"], cancellationToken);
        try
        {
            await RunScAsync(["description", ServiceName, "Prizma DPI dayanıklılık motoru"], cancellationToken);
