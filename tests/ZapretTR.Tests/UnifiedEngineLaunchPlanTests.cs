...
public sealed class UnifiedEngineLaunchPlanTests
{
    ...
    public void Create_UsesApplicationOwnedExecutable()
    {
        ...
        Assert.EndsWith(
            Path.Combine("engine", "ZapretTR.Engine.exe"),
            plan.ExecutablePath,
    ...
    public void FindMissingFiles_ReportsExecutableAndWinDivertDependencies()
    {
        ...
        Assert.Contains(missing, path => path.EndsWith("ZapretTR.Engine.exe", StringComparison.Ordinal));
    ...
    internal static ZapretProfile CreateProfile(params string[] arguments) => new()
    {
        Id = "unified-test",
